...
const emailNav = document.querySelector('.email');
...
const articles = [
  ...
  emailTab = {
    id: 1,
    etitle: "IOS is bae",
    earticleUrl: "https://onyeepeace.com"
  }
];

let html = ``;

all.addEventListener('click', articles.forEach(function(article) {
  html += `
  <article>
...
emailNav.addEventListener('click', function (params) {
  let ios = ``;
  ios += `
  <article>
  <a href = "${params.earticleUrl}">
  <p>${params.etitle}</p>
  </article>
  `;
